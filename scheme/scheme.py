...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    if procedure.use_env:
        args.append(env)
    #try to procedure and catch any error with the arguments
    try:
        return procedure.fn(*args)
    except TypeError:
        raise SchemeError("incorrect arugement type")
    # END Question 4
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return okay
    #loop through and eval every but last element in expressions
...
def make_call_frame(procedure, args, env):
    ...
    # BEGIN Question 12
    #MuProcedures creates child from of frame it is called
    if isinstance(procedure, MuProcedure):
        return Frame.make_child_frame(env,procedure.formals,args)
    #other procedures create child frame of procedure env (where it is defined)
    else:
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN Question 3
        #look in own frame and parent frames
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN Question 10
        #check that number of formals and values match
        if len(formals)!=len(vals):
            raise SchemeError("expected {0} values in val but got {1}".format(len(formals),len(vals)))
        while formals != nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 9A
        #create lambda funciton equivalent to define function
...
def do_begin_form(expressions, env):
    ...
    #last sub-expressions is in tail context (done in eval_all)
...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    #evaluable if true or false and then evaluate corresponding expression
    t_or_f=scheme_eval(expressions.first,env)
    body=expressions.second
    #second and third parts of if statements are in tail context
    if scheme_true(t_or_f):
        return scheme_eval(body.first,env, True)
    else:
        if body.second is not nil:
            return scheme_eval(body.second.first,env, True)
        else: 
            return okay
    # END Question 13
...
def do_and_form(expressions, env):
    ...
    # BEGIN Question 14B
    #short circuit when false is found
    if expressions is nil:
        return True
    while expressions.second is not nil:
        if scheme_false(scheme_eval(expressions.first,env)):
            return False
        expressions=expressions.second
    #last part of and is in tail context
    last=scheme_eval(expressions.first,env,True)
    if scheme_false(last):
        return False
    else:
        return last
    # END Question 14B
...
def do_or_form(expressions, env):
    ...
    # BEGIN Question 14B
    #short circuit when true is found
    if expressions is nil:
        return False
    while expressions.second is not nil:
        ...
        expressions=expressions.second
    #last part of or is in tail context
    last=scheme_eval(expressions.first,env,True)
    if scheme_true(last):
        return last
    return False
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_true(test):
            ...
            #non-predicate sub-expressions in cond is in tail context (done in eval_all)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN Question 16
    #create 2 lists, one of the keys and one of the values
    key, val=nil,nil
    while bindings is not nil:
        bind=bindings.first
        check_form(bind,2, 2)
        key=Pair(bind.first,key)
        val=Pair(scheme_eval(bind.second.first,env),val)
        bindings=bindings.second
    check_formals(key)
    return env.make_child_frame(key,val)
    # END Question 16
...
def check_formals(formals):
    ...
    """
    # BEGIN Question 11
    #check if there is same formal in list and if it is a legal formal
