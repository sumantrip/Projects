...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        dist = 0
        x = self.place 
        while x != hive or x.entrance != None:
            if random_or_none(x.bees) != None:
                if dist in range(self.min_range, self.max_range):
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6B
        if self.digesting <= 0 and random_or_none(self.place.bees) and self.place.bees:
